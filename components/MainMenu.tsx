...
const MainMenu: React.FC<MainMenuProps> = ({ onStart, onShowHistory, onShowTutorial, onShowPolicy }) => {
  return (
    <div className="absolute inset-0 z-50 overflow-y-auto bg-[#f0f0f0] pattern-grid">
      <div className="min-h-full flex flex-col items-center justify-center py-12">
        <div className="flex flex-col items-center transform -rotate-2 mb-12 animate-pop">
          ...
          <h1 className="text-7xl md:text-9xl font-black tracking-tighter leading-[0.85] text-center uppercase text-[oklch(0.6489_0.2370_26.9728)] drop-shadow-[10px_10px_0px_rgba(0,0,0,1)]">
            HÀNH TRÌNH<br />
            <span className="text-black bg-white px-4 border-4 border-black">TƯ TƯỞNG</span>
          </h1>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full max-w-[700px] px-6">
          <button onClick={onStart} className="col-span-1 md:col-span-2 py-8 bg-black text-white font-black text-4xl border-4 border-black neo-shadow hover:-translate-y-2 hover:-translate-x-2 transition-transform uppercase flex items-center justify-center gap-4 group">
          ...
          <button onClick={onShowPolicy} className="col-span-1 md:col-span-2 py-4 bg-gray-200 text-gray-600 font-bold text-sm border-4 border-black neo-shadow hover:bg-gray-300 uppercase flex items-center justify-center gap-2">
